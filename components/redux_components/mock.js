export const mock_surveys = [
  {
    id: 1,
    company: "Familiada",
    name: "Familiada",
    price: 120,
    shortDescription:
      "Dołącz do elitarnego grona ankietowanych ludzi Familiady",
    rate: 3.7,
    category: "Familijne",
    time: "5:00",
    description:
      "Ankieta stoworzona przez firme TVP s.a. którate umożliwia Ci odpowiadanie na pytania, które wystąpią kolejne",
  },
  {
    id: 2,
    company: "Allegro",
    name: "Allegro Smart",
    price: 150,
    shortDescription: "Dołącz do elitarnego grona ankietowanych ludzi Allegro",
    rate: 4.6,
    category: "Familijne",
    time: "5:00",
    description:
      "Ankieta spratewdza opinie klienta na temat serwisu Allegro, szczególnie uwzględniając usługę Allegro Smart",
  },
  {
    id: 3,
    company: "PWr",
    name: "System JSOS",
    price: 150,
    shortDescription: "Krótki opis ankiety, jest ona bardzo ciekawa",
    rate: 4.6,
    category: "Edukacja",
    time: "5:00",
    description:
      "Długi opis ankiety, który jest długim opisem ankiety, bo opisuje ankiete i jest dłuższy niż krótki opis ankiety",
  },
  {
    id: 4,
    company: "Firma",
    name: "Nazwa Ankiety",
    price: 150,
    shortDescription: "Krótki opis ankiety o bardzo ciekawej firmie",
    rate: 4.6,
    category: "Jeszcze inna kategoria",
    time: "5:00",
    description:
      "Długi opis ankiety, który jest długim opisem ankiety, bo opisuje ankiete i jest dłuższy niż krótki opis ankiety",
  },
  {
    id: 5,
    company: "Firma Kolejna",
    name: "Ciekawa ankieta",
    price: 111,
    shortDescription: "Krótki opis ankiety, jest ona bardzo ciekawa",
    rate: 5.0,
    category: "Zakupy",
    time: "10:00",
    description:
      "Długi opis ankiety, który jest długim opisem ankiety, bo opisuje ankiete i jest dłuższy niż krótki opis ankiety",
  },
];
export const userMockData = {
  name: "Norbert",
  email: "asdf@gmail.com",
  age: 16,
  sex: true,
  profession: "IT",
  place_of_residence: 1,
  growth: 163,
  weight: 40,
  level_of_fitness: 4,
};

export const mockUserPoints = 1231;

export const mockOwnedCoupons = [
  {
    id: 1,
    company: "Firma",
    category: "Obuwie",
    description: "10% zniżki na buty",
    code: "AXADSFX",
  },
  {
    id: 2,
    company: "Firma2",
    category: "Akcesoria",
    description: "50% zniżki na akcesoria",
    code: "HFDABZC",
  },
  {
    id: 3,
    company: "Firma3",
    category: "Sprzęt komputerowy",
    description: "40zł mniej za zakupy powyzej 20 zł",
    code: "kod_kuponu",
  },
  {
    id: 4,
    company: "Firma4",
    category: "kategoria kuponu",
    description: "opis kuponu",
    code: "kod_kuponu",
  },
  {
    id: 5,
    company: "Firma5",
    category: "kategoria kuponu",
    description: "opis kuponu",
    code: "kod_kuponu",
  },
  {
    id: 6,
    company: "Firma6",
    category: "kategoria kuponu",
    description: "opis kuponu",
    code: "kod_kuponu",
  },
];

export const mockCouponsToBuy = [
  {
    id: 1,
    company: "Firma",
    category: "Obuwie",
    description: "10% zniżki na buty",
    price: 120,
  },
  {
    id: 2,
    company: "Firma2",
    category: "Akcesoria",
    description: "50% zniżki na akcesoria",
    price: 120,
  },
  {
    id: 3,
    company: "Firma3",
    category: "Sprzęt komputerowy",
    description: "40zł mniej za zakupy powyzej 40 zł",
    price: 120,
  },
  {
    id: 4,
    company: "Firma4",
    category: "kategoria kuponu",
    description: "opis kuponu",
    price: 120,
  },
  {
    id: 5,
    company: "Firma5",
    category: "kategoria kuponu",
    description: "opis kuponu",
    price: 140,
  },
  {
    id: 6,
    company: "Firma6",
    category: "kategoria kuponu",
    description: "opis kuponu",
    price: 150,
  },
];

export const mock_surveys_company = [
  {
    id: 1,
    company: "Familiada",
    name: "Familiada",
    price: 120,
    shortDescription:
      "Dołącz do elitarnego grona ankietowanych ludzi Familiady",
    rate: 3.7,
    category: "Familijne",
    time: "5:00",
    description:
      "Ankieta stoworzona przez firme TVP s.a. którate umożliwia Ci odpowiadanie na pytania, które wystąpią kolejne",
    survey: {
      1: {
        question: "Ile masz lat?",
        answers: [
          {
            id: 1,
            name: "Mam 3 lata dużo lat i bardzo duży tekst tutaj jest",
            count: 2000,
          },
          {
            id: 2,
            name: "Mam 5 lata",
            count: 5000,
          },
          {
            id: 3,
            name: "Mam 8 lata",
            count: 13000,
          },
        ],
      },
      2: {
        question: "Ile masz pieniędzy?",
        answers: [
          {
            id: 1,
            name: "Mam 3 pieniądze",
            count: 2000,
          },
          {
            id: 2,
            name: "Mam 3 pieniądze",
            count: 5000,
          },
          {
            id: 3,
            name: "Mam 3 pieniądze",
            count: 13000,
          },
          {
            id: 4,
            name: "Mam 3 pieniądze",
            count: 10000,
          },
          {
            id: 5,
            name: "Mam 3 pieniądze",
            count: 3500,
          },
          {
            id: 6,
            name: "Mam 3 pieniądze",
            count: 3500,
          },
          {
            id: 7,
            name: "Mam 2 lata",
            count: 3500,
          },
        ],
      },
    },
  },

  {
    id: 2,
    company: "Allegro",
    name: "Allegro Smart",
    price: 150,
    shortDescription: "Dołącz do elitarnego grona ankietowanych ludzi Allegro",
    rate: 4.6,
    category: "Familijne",
    time: "5:00",
    description:
      "Ankieta spratewdza opinie klienta na temat serwisu Allegro, szczególnie uwzględniając usługę Allegro Smart",
    survey: {
      question: "Ile masz lat?",
      answers: [
        {
          id: 1,
          name: "Mam 3 lata dużo lat i bardzo duży tekst tutaj jest",
          count: 2000,
        },
        {
          id: 2,
          name: "Mam 5 lata",
          count: 5000,
        },
        {
          id: 3,
          name: "Mam 8 lata",
          count: 13000,
        },
        {
          id: 4,
          name: "Mam 9 lata",
          count: 10000,
        },
        {
          id: 5,
          name: "Mam 2 lata",
          count: 3500,
        },
      ],
    },
  },
  {
    id: 3,
    company: "PWr",
    name: "System JSOS",
    price: 150,
    shortDescription: "Krótki opis ankiety, jest ona bardzo ciekawa",
    rate: 4.6,
    category: "Edukacja",
    time: "5:00",
    description:
      "Długi opis ankiety, który jest długim opisem ankiety, bo opisuje ankiete i jest dłuższy niż krótki opis ankiety",
    survey: {
      question: "Ile masz lat?",
      answers: [
        {
          id: 1,
          name: "Mam 3 lata dużo lat i bardzo duży tekst tutaj jest",
          count: 2000,
        },
        {
          id: 2,
          name: "Mam 5 lata",
          count: 5000,
        },
        {
          id: 3,
          name: "Mam 8 lata",
          count: 13000,
        },
        {
          id: 4,
          name: "Mam 9 lata",
          count: 10000,
        },
        {
          id: 5,
          name: "Mam 2 lata",
          count: 3500,
        },
      ],
    },
  },
  {
    id: 4,
    company: "Firma",
    name: "Nazwa Ankiety",
    price: 150,
    shortDescription: "Krótki opis ankiety o bardzo ciekawej firmie",
    rate: 4.6,
    category: "Jeszcze inna kategoria",
    time: "5:00",
    description:
      "Długi opis ankiety, który jest długim opisem ankiety, bo opisuje ankiete i jest dłuższy niż krótki opis ankiety",
    survey: {
      question: "Ile masz lat?",
      answers: [
        {
          id: 1,
          name: "Mam 3 lata dużo lat i bardzo duży tekst tutaj jest",
          count: 2000,
        },
        {
          id: 2,
          name: "Mam 5 lata",
          count: 5000,
        },
        {
          id: 3,
          name: "Mam 8 lata",
          count: 13000,
        },
        {
          id: 4,
          name: "Mam 9 lata",
          count: 10000,
        },
        {
          id: 5,
          name: "Mam 2 lata",
          count: 3500,
        },
      ],
    },
  },
  {
    id: 5,
    company: "Firma Kolejna",
    name: "Ciekawa ankieta",
    price: 111,
    shortDescription: "Krótki opis ankiety, jest ona bardzo ciekawa",
    rate: 5.0,
    category: "Zakupy",
    time: "10:00",
    description:
      "Długi opis ankiety, który jest długim opisem ankiety, bo opisuje ankiete i jest dłuższy niż krótki opis ankiety",
    survey: {
      question: "Ile masz lat?",
      answers: [
        {
          id: 1,
          name: "Mam 3 lata dużo lat i bardzo duży tekst tutaj jest",
          count: 2000,
        },
        {
          id: 2,
          name: "Mam 5 lata",
          count: 5000,
        },
        {
          id: 3,
          name: "Mam 8 lata",
          count: 13000,
        },
        {
          id: 4,
          name: "Mam 9 lata",
          count: 10000,
        },
        {
          id: 5,
          name: "Mam 2 lata",
          count: 3500,
        },
      ],
    },
  },
];
