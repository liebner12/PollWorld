...
const AppNavigator = () => {
  ...
  const handleSignInShorter = async () => {
    ...
    await dispatchSurveysWithData(
      dispatchSurveysWithValue(surveys.map(serverToSurvey))
    );
        ...
        {isAuthenticated ? (
          ...
          <>
            ...
            <Stack.Screen name="Physical">
              {(props) => (
                <PhysicalPage {...props} onSignIn={handleSignInShorter} />
              )}
            </Stack.Screen>
          </>
        )}
